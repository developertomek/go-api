...
type WorkoutEntry struct {
	ID              int      `json:"id"`
	EcerciseName    string   `json:"exercise_name"`
	Sets            int      `json:"sets"`
	Reps            *int     `json:"reps"`
...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	for _, entry := range workout.Entries {
		...
		RETURNING id
		`
		err = tx.QueryRow(query, workout.ID, entry.EcerciseName, entry.Sets, entry.Reps, entry.DurationSeconds, entry.Weight, entry.Notes, entry.OrderIndex).Scan(&entry.ID)
		if err != nil {
			return nil, err
		}
	}
